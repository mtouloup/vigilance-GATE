...
class LLMParser:
    ...
    def parse_with_fields(self, raw: str | dict) -> tuple[CanonicalEvent, dict]:
        """Like parse() but also returns the raw dict of fields extracted by the LLM."""
        raw_text = str(raw) if not isinstance(raw, str) else raw
        fields = self._llm.extract_fields(raw_text, _CANONICAL_FIELDS)
        return self._build_event(raw_text, fields), fields

    def parse(self, raw: str | dict) -> CanonicalEvent:
        """Use LLM field extraction to build a CanonicalEvent."""
        raw_text = str(raw) if not isinstance(raw, str) else raw
        fields = self._llm.extract_fields(raw_text, _CANONICAL_FIELDS)
        return self._build_event(raw_text, fields)

    def _build_event(self, raw_text: str, fields: dict) -> CanonicalEvent:
