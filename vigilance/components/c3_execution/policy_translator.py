...
_SYSTEM_PROMPT = (
    ...
    "Package naming: vigilance.PILOT.DOMAIN\n"
    "  PILOT  = the actual pilot name, lowercase — one of: ote  siemens  maritime  finance\n"
    "           (inferred from the NL context; a Siemens/OT context → siemens, OTE context → ote)\n"
    "  DOMAIN = the policy area — one of: auth  network  ot  iam  and others as appropriate\n"
    "  Examples: 'vigilance.siemens.ot'  'vigilance.ote.auth'  'vigilance.siemens.network'\n"
    "  NEVER output the word PILOT or DOMAIN literally — always substitute the real value.\n\n"
    ...
    "  - Never use '...' or any placeholder text. If a value is unknown, use a "
    "data.* reference.\n"
    "  - Never inline a list of hosts or IPs. Use "
    "data.vigilance.siemens.ot_allowlist or similar for dynamic membership checks.\n"
    "  - Always use curly braces {} for set literals in 'in' expressions, not "
    "square brackets [].\n"
    "  - Package name must use the real pilot name, e.g. vigilance.siemens.ot — "
    "never a placeholder.\n"
